...
const authorisation = async function (req, res, next) {
  try {
    ...
    if (bookId === ":bookId") {
      return res.status(400).send({ status: false, message: "Please enter bookId to proceed!" });
    }
    ...
    if (!book) {
      return res.status(400).send({ status: false, message: "We are sorry; Given bookId does not exist!" });
    }
    // CASE-4: bookId exists but is deleted (isDeleted: true)
    if (book && book.isDeleted) {
      return res.status(404).send({status: false,message: "We are sorry; Given bookId does not exist" });
    }
    ...
    if (req.userId !== book.userId.toString()) {
      ...
      return res.status(401).send({
        status: false,
        message: `Authorisation Failed! You are logged in ${req.userId} not as ${book.userId}`,
      });
